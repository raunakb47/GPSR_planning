...
class GpsrPlanner:
    ...
    def __init__(
        ...
        self.llm = ChatLlamaROS(
            temp=0.10,  # Lowered for determinism
        ...
        # Refine chain prompt (NFR consideration with strict JSON output and Few-shot Examples)
        refine_prompt_template = ChatPromptTemplate.from_messages([
            SystemMessagePromptTemplate.from_template(
                "Refine the base plan by incorporating NFR constraints only if they are relevant to the action in context. "
                "Do not change existing actions, parameters from the base plan unless absolutely necessary for NFR compliance. "
                "Append to explanations of next action, NFR suggestion in very concise form, when applicable. Check the given examples. "
                "If no relevant NFR or no improvements needed, output the base plan unchanged. Keep explanations clear and concise. "
    ...
    def _is_valid_refinement(self, base_plan, refined_plan):
        ...
        if set(base_actions) - set(refined_actions):  # Check core base actions missing
            return False
